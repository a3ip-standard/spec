...
**Current version: v1.6** — Platform Conventions: Cowork skill registration. This document is the complete, self-contained specification. All versions v1.0–v1.5 are fully backward compatible with v1.6.
...
│   └── <script-key>.py
│
└── adapters/              # Optional. Platform and OS-specific overrides.
    ├── windows/
    │   └── scripts/       # PowerShell alternatives to scripts/
    ├── macos-linux/
    │   └── scripts/
    ├── claude/            # AI-platform adapter hints
    ├── codex/
    └── generic/
```
...
| **Skills** | Registered in Cowork's skill registry (see _Cowork skill registration_ below). |
...
#### Cowork skill registration

Cowork maintains a central skill registry backed by a `manifest.json` file in the skills-plugin directory. To install a skill so it appears in the Cowork Personal skills panel:

**Step 1** — Copy `SKILL.md` to the Cowork skills-plugin directory:

```
<cowork_skills_dir>/skills/<name>/SKILL.md
```

**Step 2** — Add an entry to `manifest.json` in the skills-plugin root:

```json
{
  "skillId": "skill_<name>",
  "name": "<name>",
  "description": "<trigger description — copy from SKILL.md frontmatter>",
  "creatorType": "user",
...
The `skillId` must be unique. Use the prefix `skill_` for user-created skills.
The `description` drives AI trigger matching — use the full description from the SKILL.md `description:` frontmatter field.
Cowork picks up `manifest.json` changes on next session reload; no process restart is required.

#### Cowork file operations (Windows)

Cowork runs on Windows. The Cowork Bash sandbox mounts the workspace at a Linux path, but its `~` is **not** the Windows user home (`C:\Users\<user>`). Use Windows-native tools for all file operations during skill installation:

- **Write files:** `mcp__filesystem__write_file`
- **Copy files:** `mcp__windows-cli` PowerShell `Copy-Item`
- **List directories:** `mcp__filesystem__list_directory`
- **Create directories:** `mcp__filesystem__create_directory`

Do **not** use `bash cp`, `bash mkdir`, or `bash cat` on Windows paths — these tools operate inside the sandbox, not on the user's Windows filesystem.
...
Look for `installed.json` in `{{config.install_dir}}`.

- **Not found:** proceed with the full install steps below.
- **Found:** go directly to [## Upgrading](#upgrading) at the end of this document.
...
| `1.6` | 2026-05-15 | Platform Conventions: Cowork two-step skill registration (`manifest.json` + SKILL.md copy); Cowork file operations note (Windows tools, not bash sandbox) |

All packages authored under v1.0–v1.5 remain fully valid under v1.6.
