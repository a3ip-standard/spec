...
| Dependencies | MCP server declarations | `dependencies` block + artifact / protocol component types |
| Tooling | — | `a3ip` CLI + AI-readable `INSTALL.md` |
...
Packages install from a bundle on their own. A **registry** is an optional
*distribution* layer for discovering and delivering packages — it is not part of
this standard. Registry formats and any browse, install-from-registry, or
update-checking tooling are intentionally **outside** the A3IP specification,
which covers the package's form and install contract only. Multiple independent
registries may exist; see Governance for the project's reference registry.

A package MAY record where it was installed from via the optional
`registry_source` field in `installed.json` (see [installed.json](#installedjson)).
That field is an opaque pointer — a path or URL — and this specification defines
the field, not the format of whatever it points at.
...
| `registry_source` | Opaque pointer (a path or URL) recording where this package was installed from, if a distribution registry was used. The spec defines the field only — registry formats and any update-checking are outside this standard. |
...
11. **Adopts, not forks.** Any valid SKILL.md skill is valid inside A3IP without modification. A3IP does not implement APM-manifest compatibility.
...
| Cross-harness install | — | `apm install` (deterministic CLI) | AI-run conversational install |
| Install-time governance | — | Policy engine (`apm-policy.yml`) + content-hash scan | Declared `permissions:` block (presented by AI, not enforced) |
| Configuration wizard | — | — | `CONFIGURE.md` |
| Script trust levels | — | — | `trust_level:` |
| Version pinning / upgrades | — | Lockfile + pinning | `CHANGELOG.md` + `installed.json` delta upgrades |
| Sensitive config handling | — | — | `sensitive:` + `storage:` |
| Plain-text single-file bundle | — | git-repo model | `.a3ip.bundle` |
| Outcome-shaped install contract | — | — | Tier 2 outcomes + runtime adapters (v1.9+) |

APM is a mature dependency manager with a real installer, content-hash lockfiles, security scanning, and an enforced policy engine across seven coding harnesses. The rows where APM shows `—` are capabilities A3IP frames differently (a *conversational*, AI-run install), not gaps in APM. For developer workflows the two overlap heavily, and APM is the more complete, better-supported option.
...
| `1.11` | 2026-07-11 | Scope reduction: removed the `## Registry` section (registry.yaml format, browse, install-from-registry, update checks) and the Discovery capability-table row. Registries are a distribution concern, not part of the standard; `registry_source` retained as an opaque optional `installed.json` field. No package-format or install-contract change. |
| `1.12` | 2026-07-18 | Accuracy pass: corrected the description of Microsoft APM to its current scope (a dependency manager and installer with a content-hash lockfile, security scanning, and an enforced policy engine across seven harnesses); reworded Principle 11 from "superset" to "adopts" and removed the never-implemented APM-manifest-compatibility claim; made the Relationship-to-Existing-Standards table's APM column fair and dropped the stale `registry.yaml` row (registry removed in v1.11); corrected the intro table's "APM CLI compatible" / "APM apm.yml model" rows. No normative or package-format change. |

All packages authored under v1.0–v1.11 remain fully valid under v1.12.

---

*A3IP Specification v1.12 — Complete consolidated edition*
